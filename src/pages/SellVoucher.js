...
import { sellVoucher, getClients } from "../api/clients";
...
function SellVoucher() {
  ...
  const handleSellVoucher = async (id, amount) => {
    if (isSubmitting) return;

    const numericAmount = Number(amount);
    if (!id || !amount || isNaN(numericAmount) || numericAmount <= 0) {
      toast.warn('Informe um valor vAlido para adicionar.');
      return;
    }

    try {
      setIsSubmitting(true);
      const response = await sellVoucher(id, amount);
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Erro ao adicionar saldo';
  ...
  return (
    <>
      ...
      <main className="px-10 py-6">
        ...
        <div className="">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Adicionar Voucher</h1>
            <p className="text-sm text-gray-500 mt-1">Gerencie e adicione saldo para os clientes cadastrados</p>
          </div>
        </div>
        ...
        <div data-testid="voucher-operation-modal" className={` ${showModal ? 'block' : 'hidden'} fixed inset-0 bg-black/40 flex items-center justify-center`}>
          <div className=" bg-white rounded-2xl shadow-2xl p-8">
            <div className=" flex items-center justify-between mb-6">
              <div>
                <h2 className="text-lg font-bold text-gray-800">Adicionar Voucher</h2>
            ...
            <div className="mb-4">
              <label className="block text-sm font-semibold text-gray-700 mb-2">Valor a Adicionar</label>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={() => handleSellVoucher(selectedClient?.id || selectedClient?._id, amount)}
